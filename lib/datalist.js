...
export async function jDataFamily() {
  ...
  export async function groupByFamily() {
    ...
    familyData.forEach(familyDoc => {
      ...
      console.log('working on familyDoc:', familyDoc);
  
     
       //  get the clan head name directly from `allCharacterData`  
   const clanHead = ( headOfClanId !== null) ? allCharacterData.filter(characterObj => characterObj.id.toString() === headOfClanId)[0] : null;
  //       console.log(`clan head ${familyDoc.id}:`, clanHead);
      
  // console.log('clanHead = ' , clanHead)
      ...
      // use `familyDoc.id` as the key - place in familyGroups
